...
function renderMenu() {
  for(const category in menuData){
    ...
    menuData[category].forEach(item=>{
      if(Array.isArray(item[1])){ // Ice Cream with options
        ...
        div.innerHTML = `
          ...
          <div class="prices">
            ${options.map(opt=>`
              <button onclick="addToCart('${name}', ${opt.price}, '${opt.label}')">
                ${opt.label} – ${opt.price} EGP
              </button>
            `).join("")}
          </div>
        `;
      ...
      div.innerHTML = `
        ...
        ${
          price2
            ? `<div class="prices">
                <button onclick="addToCart('${name}', ${price1}, 'Medium')">
                  Medium – ${price1} EGP
                </button>
